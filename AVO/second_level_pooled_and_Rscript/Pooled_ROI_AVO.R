...
read.subject.roi <- function(subjroi) {
   subj <- subjroi[[1]]
   roi <- subjroi[[2]]
   filename <- paste0("derivatives/afni/",subj,"/Avoidance/FL_results_ROI_pooled/betas_ROI_",roi,"_REML.1D")
...
write.table(X, file="derivatives/afni/SL/AVO/SL_pooled_ROI_AVO.csv", sep=",", row.names=FALSE)
