...
read.subject.roi <- function(subjroi) {
   ...
   roi <- subjroi[[2]]
   tryCatch({
	   filename <- paste0("derivatives/afni/",subj,"/Avoidance/FL_results_ROI_parametric_modulation/betas_ROI_",roi,"_REML.1D")
...
write.table(X, file="derivatives/afni/SL/AVO/SL_PM_ROI_AVO.csv", sep=",", row.names=FALSE)
