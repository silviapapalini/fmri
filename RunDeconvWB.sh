# DESCRIPTION: this script allows only to calculate the beta values for each condition and trial (csm csav for the whole brain). I did this to have the results from the whole brain> For the betas values from each ROI check the output from the other deconv script.
...
' $all_confounds $prefix/confounds
	...
	3dDeconvolve -input $input \
	    ...
	    -glt_label 2 csm \
	    -fout -tout \
	    -x1D $prefix/XWB.xmat.1D \
	    -xjpeg $prefix/XWB.jpg \
	    -sresp 1 $prefix/sresp_csavWB \
	    -sresp 2 $prefix/sresp_csmWB \
	    -fitts $prefix/fittsWB \
	    -errts $prefix/errtsWB \
	    -bucket $prefix/statsWB
	  mv 3dDeconvolve.err "$prefix/3dDeconvolveWB.err"
done

#           contrasts if pooling

#	    -gltsym 'SYM: csav -csm' \
#	    -glt_label 1 csav-csm WB \
#	    -gltsym 'SYM: csm -csav' \
#	    -glt_label 2 csm-csav WB\
