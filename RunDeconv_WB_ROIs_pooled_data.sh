...
3dresample -master $input \
  ...
  -inset "derivatives/ROIs/NAcc_bil.nii" \
  -rmode NN
fi
...
' $all_confounds $prefix/confounds
	...
	3dDeconvolve -input $input \
	  ...
	  mv 3dDeconvolve.err "$result_WB_prefix/3dDeconvolveWB.err"
	  
	  for roi in $ROIs; do
	    roi_mask="derivatives/ROIs/${roi}_resam.nii"
	    3dmaskave -quiet -mask $roi_mask $result_ROI_prefix/statsWB+tlrc[10,13,16,19] > "$result_ROI_prefix/averaged_betas_from_$roi"
	  done
done
