...
names <- readLines("~/fmri-process/sub-bricks-from-FL")
...
read.subject.roi <- function(subjroi) {
   subj <- subjroi[[1]]
   roi <- subjroi[[2]]
   filename <- paste0("derivatives/afni/",subj,"/Extinction/FL_results_ROI_non_pooled/betas_ROI_",roi,"_REML.1D")
   ...
   Y %>% tidyr::pivot_longer(tidyr::everything()) %>%
 	 ...
 	 dplyr::mutate(
 	 	CS=factor(CS, levels=c("csav", "csm", "csunav"), labels=c("CSav", "CSM", "CSunav")),
...
write.table(X, file="derivatives/afni/SL/SL_non_pooled_roi.csv", sep=",", row.names=FALSE)
