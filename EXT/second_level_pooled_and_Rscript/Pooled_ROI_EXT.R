...
read.subject.roi <- function(subjroi) {
   subj <- subjroi[[1]]
   roi <- subjroi[[2]]
   filename <- paste0("derivatives/afni/",subj,"/Extinction/FL_results_ROI_pooled/betas_ROI_",roi,"_REML.1D")
...
write.table(X, file="derivatives/afni/SL/EXT/SL_pooled_ROI_EXT.csv", sep=",", row.names=FALSE)
