...
prefix <- args[2]
cat(paste(X[["relief_csm"]]$onset, X[["relief_csm"]]$relief_rating, sep="*"), sep=" ", file=file(paste0(prefix, "/relief_csm_modulated.1D")))
cat(paste(X[["relief_csav"]]$onset, X[["relief_csav"]]$relief_rating, sep="*"), sep=" ", file=file(paste0(prefix, "/relief_csav_modulated.1D")))
...
' $all_confounds $prefix/confounds
	...
	3dDeconvolve -input $input \
	    ...
	    -local_times \
	    -num_stimts 5 \
	    -stim_times_AM2 1 "$prefix/relief_csav_modulated.1D" 'GAM' \
